...
class Unit:
    def __init__(self, position, allegiance, units: Set[Unit], walls: Set[Point], attack_power = 3):
    ...
    def attack(self, enemy: Unit) -> bool:
        ...
        if enemy.allegiance == "E" and result:
    ...
    for y, line in enumerate(input.splitlines()):
        for x, c in enumerate(line):
            if c == "#":
                ...
                units.add(Unit(Point(x, y), c, units, walls, attack_power=elf_attack))
    ...
    try:
        ...
        while not battle_over:
            for unit in sorted(units, key=lambda unit: (unit.position.y, unit.position.x)):
                ...
                if battle_over:
                    return rounds * sum([unit.health for unit in units])
            # pretty_print(units, walls)
            rounds += 1
    except ElfDeadException as e:
        return -1
...
while result == -1:
    elf_attack += 1
    # print("checking elf attack", elf_attack)
    result = battle(elf_attack)
