...
def add_point(p: Point, q: Point):
    return Point(x = p.x + q.x, y = p.y + q.y, z = p.z + q.z, w = p.w + q.w)
...
for y, line in enumerate(input_string.splitlines()):
    for x, c in enumerate(line):
        if c == "#":
            active_cells.add(Point(x, y, 0, 0))
...
def pretty_print(active: Set[Point]):
    ...
    min_w = min(active, key=lambda a: a.w).w
    max_w = max(active, key=lambda a: a.w).w
